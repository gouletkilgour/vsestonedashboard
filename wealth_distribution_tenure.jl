...
# Dimensions used (coordinate = "1.<stat>.<tenure>.<wealth>.0.0.0.0.0.0"):
#   2 Statistics      -> Value (1), Distribution of value (2), Value per household (3)
#   3 Characteristics -> housing tenure (45-48)
...
const TENURES = [
    "Owner"                 => 45,
    "Owner with mortgage"    => 46,
    "Owner without mortgage" => 47,
    "Renter"                 => 48,
]
...
coordinate(stat_id, tenure_id, wealth_id) = "1.$(stat_id).$(tenure_id).$(wealth_id).0.0.0.0.0.0"

println("Looking up vector IDs for wealth distribution by housing tenure series...")

combo_ids = [(tid, sid, wid) for (_, tid) in TENURES for (_, sid) in STATS for (_, wid) in WEALTH_TYPES]

coords_payload = [Dict("productId" => PRODUCT_ID, "coordinate" => coordinate(sid, tid, wid))
                   for (tid, sid, wid) in combo_ids]
...
for (tenure, tid) in TENURES, (stat, sid) in STATS, (wealth, wid) in WEALTH_TYPES
    vid = vector_for_coord[coordinate(sid, tid, wid)]
    push!(vector_ids, vid)
    key_for_vector[vid] = (tenure, stat, wealth)
end
...
for (tenure, _) in TENURES
    data[tenure] = Dict{String, Dict{String, Dict{String, Vector}}}()
    for (stat, _) in STATS
        data[tenure][stat] = Dict{String, Dict{String, Vector}}()
    end
end
...
for item in data_parsed
    ...
    tenure, stat, wealth = key_for_vector[vid]
    ...
    data[tenure][stat][wealth] = Dict("x" => xs, "label" => labels, "y" => ys)
end
...
<head>
  ...
  <title>Wealth Distribution and Composition by Housing Status</title>
...
<header>
  <h1>Wealth Distribution and Composition by Housing Status</h1>
</header>
...
<main>
  <section>
    ...
    <div class="prose">
      ...
      <p>
        This chart shows how household assets, liabilities, and net worth (wealth) are distributed across households, grouped by housing status.
      </p>

      <h2>Unit of Analysis</h2>
      <p>
        These series are based on household (not individual) wealth. Households are grouped by housing status: owner (with or without a mortgage), owner with mortgage, owner without mortgage, and renter. 
      </p>

      <h2>Statistics</h2>
      <p>
        <strong>Value</strong> is the aggregate dollar amount held by all households in a given tenure group, expressed in millions of dollars.
      </p>
      <p>
        <strong>Distribution of value</strong> is the share of the national total held by a given tenure group, expressed as a percentage.
      </p>
      <p>
        <strong>Value per household</strong> is the average dollar amount per household within a given tenure group.
      </p>
  ...
  <section>
    ...
    <div class="chart-controls">
      <div class="ctrl-group">
        <span class="chart-ctrl-label">Housing status:</span>
        <select id="tenure-select">
          <option value="Owner" selected>Owner</option>
          <option value="Owner with mortgage">&nbsp;&nbsp;Owner with mortgage</option>
          <option value="Owner without mortgage">&nbsp;&nbsp;Owner without mortgage</option>
...
<script>
  ...
  function buildBarTrace(tenure, stat, wealth, freq, color, labelOverride) {
    const s = filterByFrequency(DATA[tenure][stat][wealth], freq);
  ...
  function buildTotalLine(tenure, stat, wealth, freq) {
    const s = filterByFrequency(DATA[tenure][stat][wealth], freq);
    return {
      x: s.x,
      y: s.y,
  ...
  function buildTrace(tenure, stat, wealth, freq) {
    ...
    if (wealth === "Net worth (wealth)" && stat !== "Distribution of value") {
      const bars = withHoverMarkers([
        buildBarTrace(tenure, stat, "Total assets", freq, COMPONENT_COLORS[0]),
        buildBarTrace(tenure, stat, "Total liabilities", freq, COMPONENT_COLORS[1]),
      ], false);
      return bars.concat([buildTotalLine(tenure, stat, wealth, freq)]);
    }
    ...
    if (decomp && stat !== "Distribution of value") {
      const bars = withHoverMarkers(decomp.map((component, i) =>
        buildBarTrace(tenure, stat, component, freq, COMPONENT_COLORS[i])
      ));
      return bars.concat([buildTotalLine(tenure, stat, wealth, freq)]);
    }
    const s = filterByFrequency(DATA[tenure][stat][wealth], freq);
  ...
  function buildLayout(tenure, stat, wealth, freq) {
    ...
    var s = filterByFrequency(DATA[tenure][stat][wealth], freq);
    var tickvals = [], ticktext = [];
    s.label.forEach(function(lab, i) {
      if (lab.endsWith("Q4")) { tickvals.push(s.x[i]); ticktext.push(lab); }
    });
    return {
      title: { text: titleCase(wealth.replace(" (wealth)", "")) + " \\u2013 " + titleCase(tenure) + " \\u2013 Canada" },
  ...
  function update() {
    Plotly.react("chart", buildTrace(currentTenure, currentStat, currentWealth, currentFreq), buildLayout(currentTenure, currentStat, currentWealth, currentFreq), { responsive: true });
  }
  ...
  Plotly.newPlot("chart", buildTrace(currentTenure, currentStat, currentWealth, currentFreq), buildLayout(currentTenure, currentStat, currentWealth, currentFreq), { responsive: true });

  var resizeTimer;
  new ResizeObserver(function() {
    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(function() {
      Plotly.relayout("chart", buildLayout(currentTenure, currentStat, currentWealth, currentFreq));
    }, 50);
...
output = joinpath(@__DIR__, "wealth_distribution_tenure.html")
write(output, html)
